/* instruments-tiles.js — the eight HUD tiles. instruments.js owns the banks,
   the layout, the alarms and the paint loop; this file owns what each tile
   draws and the readings it integrates. Register with the framework, which
   calls back with `F`, the shared kit it needs. */
(function () {
  const { approach, clamp, wrapPi } = Util;

  window.Instruments.registerTiles(function (F) {
    const { LAMP, COLD, DIM, BAD, WARN, GOOD, setFont, mono, spark, fit, fmtK } = F;
