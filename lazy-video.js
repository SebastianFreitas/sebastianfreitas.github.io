...
(function () {
  ...
  videos.forEach((v) => {
    // Controls on every clip. The terminal text is small in an inline
    // player, and fullscreen is the only way anyone actually reads it.
    v.controls = true;
    ...
    (entries) => {
      for (const entry of entries) {
        ...
        if (entry.isIntersecting) {
          ...
          v.play().catch(() => {});   // blocked autoplay: the play button is already there
        } else {
          v.pause();
        }
      }
