...
window.Instruments = (function () {
  ...
  function radar(p, r, dt) {
    ...
    for (let i = 0; i < 14; i++) {
      const a = ang - i * 0.055;
      ctx.strokeStyle = `rgba(${LAMP},${0.16 * (1 - i / 14)})`;
    ...
    for (const m of (r.marks || [])) {
      ...
      const col = m.claimed ? DIM : LAMP;
      ctx.fillStyle = `rgba(${col},${0.25 + 0.75 * lit})`;
      ctx.beginPath(); ctx.arc(bx, by, m.claimed ? 1.6 : 2.3, 0, 6.283); ctx.fill();
      if (!m.claimed && lit > 0.25) {
        ctx.strokeStyle = `rgba(${LAMP},${lit * 0.5})`;
        ctx.beginPath(); ctx.arc(bx, by, 3 + (1 - lit) * 7, 0, 6.283); ctx.stroke();
      }
    }
  ...
  function signal(p, r, dt) {
    ...
    for (let i = 0; i < BANDS; i++) {
      ...
      const x = 5 + i * bw;
      ctx.fillStyle = `rgba(${COLD},${0.30 + 0.5 * bars[i]})`;
      ctx.fillRect(x, inner - 6 - h, bw - 1.6, h);
      ctx.fillStyle = `rgba(${LAMP},0.45)`;
