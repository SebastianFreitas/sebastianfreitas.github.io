...
`js/genesis/genesis-oldones.js` (1254), `js/pages/voidscape.js` (849),
...
| `genesis-circle.js` | 178 | `circle` beat inside: roles end state as backdrop; a ring drawn in 8 held arcs, grey dust creeps to a dune in its lower third, baked grey strands spiral in, one rose thread from the colour pool, then a snap to a dull flat disc (the first universe): `draw` | `window.GenCircle` |
| `genesis-chains.js` | 181 | `chains` beat inside: the P8 disc snaps to a ring, linked rings snap out along a chain with the rose thread, then 3 held zoom-outs to hex chain-mail fields (levels 2-3 stamped from offscreen tiles), magic rings carry a pale chip: `draw` | `window.GenChains` |
| `genesis-corrupt.js` | 235 | `corrupt` beat inside: a level-1 ring field, 3 rings turn red in held arc steps, a dark blade cuts each loose, a dark crescent (mothers) folds it to grey dust, one red dot per ring rises in steps into a lumpy red layer at the roof: `draw` | `window.GenCorrupt` |
| `genesis-rexin.js` | 277 | `rex` beat inside: Rex tears the level-1 ring into two arcs, grows over held zoom-outs (field shrinks, roof band thins) until only part of him fits, then the chains settle round him as braided dark-red rivers; `draw(...,reach)` bends the right arm, `rexGeom(W,H,reach).hand`: `draw`, `rexGeom` | `window.GenRexIn` |
| `genesis-dot.js` | 98 | `dot` beat inside: over GenRexIn at t=99, a ring turns red, the red dot (Obrokxus) steps out towards the right wall while Rex's right arm reaches in held steps: `draw` | `window.GenDot` |
| `genesis-clash.js` | 273 | `clash` beat inside: close view, Rex's fist drives the red dot into the womb wall in 4 blows (stepped octagon rings), the wall bulges and tears in held steps onto tier-1 strata: `draw` | `window.GenClash` (`draw(...,noActors)`, `geom`) |
| `genesis-death.js` | 171 | `death` beat: clash backdrop without actors, Rex's fist steps into the tear and curls over the red dot, the arm steps to dead grey. `window.GenDeath` |
| `genesis-gods-beat.js` | 40 | `gods` beat opening: the death frame cut by a stepped octagon onto the live land. `window.GenGodsBeat` |
...
| `genesis-rex.js` | 506 | `chaseAt chaseFightAt ringFightAt`, the ending `escapeAt escapeDrift escapeCam escapeCamLead` (the worm gets away west, Ormius follows alone), `drawRexLand` (hot rock cooling to slate, fissures), `drawRexHole`, `drawRexGrip`, `drawBuried` (the eye in the ground), `drawGodsBirth` (five gods walk west inside the land to the deck, each with a chip of Rex) | `window.GenRex` |
...
| Plan phase state (one phase per prompt, then `/clear`) | `PLAN_STATE.md` at the repo root, committed, exists only while a plan runs; format and hard-stop message in `.claude/skills/plan/SKILL.md` "one phase per prompt" |
