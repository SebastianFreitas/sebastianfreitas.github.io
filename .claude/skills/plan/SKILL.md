...
1. Create `.claude/plans/<name>.md` from TEMPLATE, write `<name>` to
   ...
   and say the new plan needs its own worktree), paste the owner's brief **verbatim** under Brief (condense
...
Set `Stage: done`, delete this checkout's HERE, list every `D<n> (auto)` in the report
