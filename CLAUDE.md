...
| `voidship-prow.js` | 305 | The red prow, the artifact the ship was built around: conduits into the hull, the rust mass with grey clamps, the dark reflective column with branching strings, the claw arm (far copy behind), the leg; head-on version too. Geometry tables `MASS COL STRINGS UA FA THIGH SHIN FOOT`, palette `RUST_* OBS_*`; reads `ship.face`, `ship.strain`; `Util.reduced()` freezes the sway/pulse | extends `window.VoidshipArt {drawProwBack, drawProw, drawProwFront, PROW}` |
